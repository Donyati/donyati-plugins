...
| `/donyati-contribute` | Submit an accelerator, internal doc or case study for review, resubmit one a reviewer sent back, or list your submissions and where each stands |

> **`/donyati-case-study` is a Claude Code skill.** It drafts the two documents locally, which
> needs Claude Code's file access, so it will not show up in Desktop — that is expected rather
> than a broken install. To submit the draft, use `/donyati-contribute` (it calls
> `submit_case_study` and works on every surface), or paste it into
> <https://expert-agents.donyati.com/case-studies/new>.
