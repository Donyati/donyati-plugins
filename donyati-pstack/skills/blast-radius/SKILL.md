...
Scoped narrowly. Not what the code does, and not why it was built that way. Only what this change breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR and commits with `az repos pr show` and `git log -p` — Donyati repos are Azure DevOps, not GitHub.
...
6. For a big or wide change, put the same question to a different model family and merge the answers: `dispatch codex` or `dispatch gemini`. Different families catch different real bugs. Do not fan out Claude subagents for this — the value is the disagreement, not a second opinion.
